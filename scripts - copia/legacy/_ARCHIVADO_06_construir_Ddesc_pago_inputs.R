...
#
# PASO A — D_desc (estructura organizativa)
...
#   - se comprueba si df_final ya viene con duplicados (NCODI, anyo)
#   - se deduplica map_cnh por NCODI antes del join
#   - se informa si el join multiplica filas
# ============================================================
...
print_dups_key <- function(data, key = c("NCODI", "anyo"), max_n = 50) {
  dups <- data %>%
    dplyr::count(dplyr::across(dplyr::all_of(key)), name = "n") %>%
    dplyr::filter(n > 1) %>%
    dplyr::arrange(dplyr::desc(n), dplyr::across(dplyr::all_of(key)))

  if (nrow(dups) > 0) {
    print(dups, n = min(max_n, nrow(dups)))
  }
  invisible(dups)
}
...
if (!file.exists(DF_FINAL_RDATA_PATH)) {
  stop(
    "No se encontró df_final en: ", DF_FINAL_RDATA_PATH,
    "\nEjecuta el script anterior antes.",
    call. = FALSE
  )
}

load(DF_FINAL_RDATA_PATH)
message("DF_FINAL_RDATA_PATH = ", DF_FINAL_RDATA_PATH)
message("Última modificación del RData: ", file.info(DF_FINAL_RDATA_PATH)$mtime)

df_final$NCODI <- to_chr(df_final$NCODI)
...
message("Duplicados iniciales en df_final: ", nrow(dups_ini))
if (nrow(dups_ini) > 0) {
  print(utils::head(dups_ini, 20))
}
if (!exists("df_final")) {
...
# Normalizar clave básica por seguridad
df_final$NCODI <- to_chr(df_final$NCODI)
df_final$anyo  <- suppressWarnings(as.integer(df_final$anyo))

# Comprobar duplicados ya al cargar
dups_ini <- df_final %>%
  dplyr::count(NCODI, anyo, name = "n") %>%
  dplyr::filter(n > 1)

if (nrow(dups_ini) > 0) {
  message("\nERROR: df_final ya viene con duplicados por (NCODI, anyo).")
  print(dups_ini, n = min(50, nrow(dups_ini)))
  stop(
    "Duplicados iniciales en df_final: ",
    sum(dups_ini$n - 1),
    "\nCorrige el script anterior antes de continuar.",
    call. = FALSE
  )
}
...
if (file.exists(NCODI_HOSPITAL_MAP_PATH)) {
  ...
  # Diagnóstico de duplicados en mapa
  dup_map <- map_cnh %>%
    dplyr::count(NCODI, name = "n") %>%
    dplyr::filter(!is.na(NCODI), n > 1)

  message("  NCODI duplicados en map_cnh antes de deduplicar: ", nrow(dup_map))
  if (nrow(dup_map) > 0) {
    print(dup_map, n = min(50, nrow(dup_map)))
  }

  # Deduplicar por NCODI para evitar multiplicación de filas en left_join
  map_cnh <- map_cnh %>%
    dplyr::arrange(NCODI) %>%
    dplyr::distinct(NCODI, .keep_all = TRUE)

  if (length(vars_map) > 1) {
    n_before_join <- nrow(df)
    ...
    message("  Filas antes del join: ", n_before_join)
    message("  Filas después del join: ", n_after_join)

    if (n_after_join != n_before_join) {
      warning(
        "El left_join() ha cambiado el número de filas. Revisa map_cnh y la clave NCODI.",
        call. = FALSE
      )
    }
  ...
  warning(
    "No se encontró ncodi_hospital_map.csv; se usará solo la info ya presente en df_final.",
    call. = FALSE
  )
}

# Verificación tras join
dups_after_join <- df %>%
  dplyr::count(NCODI, anyo, name = "n") %>%
  dplyr::filter(n > 1)

if (nrow(dups_after_join) > 0) {
  message("\nERROR: han aparecido duplicados en df tras el join con map_cnh.")
  print(dups_after_join, n = min(50, nrow(dups_after_join)))
  stop(
    "Duplicados tras join en df: ",
    sum(dups_after_join$n - 1),
    call. = FALSE
  )
}
...
df$D_desc_fuente <- dplyr::case_when(
  df$D_desc_cnh == 1L & df$D_desc_siae == 0L ~ "CNH_privado_prioritario_sobre_SIAE_publico",
...
if (!("altTotal_bruto" %in% names(df)))
  stop("'altTotal_bruto' no existe. Ejecuta el script anterior antes.", call. = FALSE)
...
if ("I_totIngresos" %in% names(df)) {
  ...
  denom_ingr <- rep(NA_real_, nrow(df))
...
if (nrow(dups_final) > 0) {
  message("\nERROR: duplicados finales detectados en df_final.")
  print(dups_final, n = min(50, nrow(dups_final)))
  stop(
    "Duplicados (NCODI, anyo) en df_final: ",
    sum(dups_final$n - 1),
    call. = FALSE
  )
}
