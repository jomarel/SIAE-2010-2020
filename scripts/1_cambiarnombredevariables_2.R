...
if (length(missing_pkgs) > 0) {
  stop(
    ...
    "\nInst·lalos manualmente con install.packages()."
  )
}
...
infer_formacion_header <- function() {
  ...
  for (yy in candidate_years) {
    ...
    for (f in files) {
      ...
      # HeurÌstica: cabecera contiene letras y separador ';'
      if (grepl(";", line1, fixed = TRUE) && grepl("[A-Za-z¡…Õ”⁄·ÈÌÛ˙—Ò_]", line1)) {
  ...
  stop("No se pudo inferir de forma segura la cabecera para FormaciÛn (mÛdulo 07).")
}
...
read_txt_file <- function(path, year) {
  ...
  has_header <- (length(line1) > 0) &&
    nzchar(line1) &&
    grepl("[A-Za-z¡…Õ”⁄·ÈÌÛ˙—Ò_]", line1) &&
    grepl(";", line1, fixed = TRUE)

  # Caso especial conocido: 2015 FormaciÛn sin cabecera
...
read_xml_file <- function(path) {
  ...
  if (is.na(record_tag)) stop("No se encontrÛ nodo de registro en XML: ", path)
...
standardize_names <- function(df) {
  ...
  names(df)[names(df) == "AÒo"] <- "anyo"
  names(df)[names(df) == "aÒo"] <- "anyo"
  names(df)[names(df) == "Anyo"] <- "anyo"
  names(df)[names(df) == "A√±o"] <- "anyo"
  names(df)[names(df) == "a√±o"] <- "anyo"
...
for (yy in years) {
  ...
  for (f in files) {
    ...
    # Si falta anyo (caso conocido: obstÈtrica XML 2018/2019), usar aÒo de carpeta
    if (!("anyo" %in% names(df))) {
      df$anyo <- year_num
    } else {
