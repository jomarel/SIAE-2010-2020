...
#
# Diseño del script:
# - Lee df_sfa desde data_intermediate/ o desde el directorio de trabajo.
# - Identifica de forma flexible nombres alternativos de variables.
# - Limpia índices panel (NCODI, anyo) antes de construir pdata.frame.
# - Estima los diseños A, B, C y D.
# - Guarda modelos, coeficientes, eficiencias y diagnósticos de limpieza.
#
# NOTA:
# - Este script usa panel SFA con {sfa}.
# - Por defecto utiliza TRE_Z (modelo transitorio con determinantes de
#   ineficiencia). Si quieres otro, define la variable de entorno:
#   Sys.setenv(SFA_MAIN_MODEL = "GTRE_Z")
# ------------------------------------------------------------
...
coerce_year <- function(x) {
  ...
  x0 <- trimws(x0)
  x0[x0 == ""] <- NA_character_
  suppressWarnings(as.integer(x0))
}
...
complete_subset <- function(df, vars) {
  vars <- intersect(vars, names(df))
  if (length(vars) == 0) return(rep(TRUE, nrow(df)))
...
df <- as_tibble(df) %>%
  mutate(
    NCODI = trim_na_character(NCODI),
    anyo  = coerce_year(anyo),
    .row_id = row_number()
  )
...
# ------------------------------------------------------------------
MODEL_NAME <- Sys.getenv("SFA_MAIN_MODEL", unset = "TRE_Z")
...
clean_panel_index <- function(dat, id_var = "NCODI", time_var = "anyo", context = "") {
  if (!id_var %in% names(dat) || !time_var %in% names(dat)) {
    stop("No encuentro variables de índice panel: ", id_var, " / ", time_var, call. = FALSE)
  }

  dat <- as_tibble(dat)
  dat <- dat %>%
    mutate(
      !!id_var := trim_na_character(.data[[id_var]]),
      !!time_var := coerce_year(.data[[time_var]])
    )
  ...
  if (nrow(bad_idx) > 0) {
    out_bad <- file.path(DIAG_DIR, paste0("indices_panel_descartados", if (nzchar(context)) paste0("_", gsub("[^A-Za-z0-9]+", "_", context)) else "", ".csv"))
    readr::write_csv(bad_idx, out_bad)
  }
  ...
  if (nrow(dat) == 0) {
    stop(
      "Tras limpiar los índices del panel no queda ninguna observación útil",
      if (nzchar(context)) paste0(" (", context, ")") else "",
      ". Revisa NCODI y anyo en df_sfa.",
      call. = FALSE
    )
  }
...
coerce_panel <- function(dat, id_var = "NCODI", time_var = "anyo") {
  if (anyNA(dat[[id_var]]) || anyNA(dat[[time_var]])) {
    stop(
      "Persisten NA en los índices del panel (", id_var, ", ", time_var,
      ") justo antes de pdata.frame().",
      call. = FALSE
    )
  }
...
fit_psfm_safe <- function(dat, lhs, model_name = MODEL_NAME, id_var = "NCODI", time_var = "anyo",
                          z_rhs = z_formula_rhs, pso = FALSE) {
  use_vars <- unique(c(lhs, frontier_terms, z_terms, var_ccaa, id_var, time_var, ".row_id"))
  ...
  keep_x <- complete_subset(work, frontier_terms)
  keep_z <- complete_subset(work, z_terms)
  work <- work[keep_x & keep_z, , drop = FALSE]

  work <- clean_panel_index(work, id_var = id_var, time_var = time_var, context = lhs)
...
coef_table_psfm <- function(obj) {
  ...
  if (!is.null(fit$coefficients)) {
    return(tibble(term = names(fit$coefficients), estimate = unname(fit$coefficients)))
  }
...
te_table_psfm <- function(obj, label) {
  ...
  te <- NULL
  if (!is.null(fit$efficiency)) {
    te <- as.numeric(fit$efficiency)
  ...
  if (is.null(te)) {
    return(dat %>% transmute(modelo = label, NCODI, anyo, .row_id, TE = NA_real_))
  }
...
write_model_bundle <- function(obj, label) {
  saveRDS(obj$model, file.path(MODELS_DIR, paste0(label, ".rds")))
  readr::write_csv(coef_table_psfm(obj), file.path(TABLES_DIR, paste0(label, "_coef.csv")))
  readr::write_csv(te_table_psfm(obj, label), file.path(TABLES_DIR, paste0(label, "_te.csv")))
}
...
# ------------------------------------------------------------------
# 5) Diseño D: función de distancia output
#    y_odf = -ln(altQ), ratio = altM / altQ
# ------------------------------------------------------------------
if (!is.null(var_altQ) && !is.null(var_altM) && !is.null(var_y_q)) {
  ...
  frontier_terms_D <- unique(c("ln_ratio_outputs", frontier_terms))
  frontier_terms_D <- frontier_terms_D[frontier_terms_D %in% names(dfD)]
  x_formula_rhs_D <- paste(frontier_terms_D, collapse = " + ")

  use_vars <- unique(c("y_odf", frontier_terms_D, z_terms, var_ccaa, "NCODI", "anyo", ".row_id"))
  use_vars <- use_vars[use_vars %in% names(dfD)]

  work <- dfD %>%
    select(all_of(use_vars)) %>%
    filter(is.finite(y_odf))

  keep_x <- complete_subset(work, frontier_terms_D)
  keep_z <- complete_subset(work, z_terms)
  work <- work[keep_x & keep_z, , drop = FALSE]
  work <- clean_panel_index(work, id_var = "NCODI", time_var = "anyo", context = "Diseno_D")

  pdat <- coerce_panel(work, id_var = "NCODI", time_var = "anyo")
  ...
  results$D_odf <- list(model = fit, data = work, formula = fml, model_name = MODEL_NAME)
...
if (!is.null(var_y_q) && !is.null(var_y_m)) {
  keep_vars_C <- unique(c("NCODI", "anyo", ".row_id", var_ccaa, var_y_q, var_y_m,
                          frontier_terms, z_terms, var_ddesc, var_pctsns, var_shareq))
  keep_vars_C <- keep_vars_C[keep_vars_C %in% names(df)]

  dfC <- df %>%
    select(all_of(keep_vars_C)) %>%
  ...
  keep_x <- complete_subset(work, frontier_terms)
  keep_z <- complete_subset(work, z_terms_C)
  work <- work[keep_x & keep_z, , drop = FALSE]
  work <- clean_panel_index(work, id_var = "panel_id", time_var = "anyo", context = "Diseno_C")
...
# ------------------------------------------------------------------
# 7) Tabla resumen y diagnóstico general
# ------------------------------------------------------------------
summary_tbl <- imap_dfr(results, function(obj, nm) {
  tibble(
    modelo = nm,
    paquete = "sfa::psfm",
    model_name = obj$model$model_name %||% obj$model_name %||% NA_character_,
...
readr::write_csv(summary_tbl, file.path(TABLES_DIR, "00_resumen_modelos_sfa.csv"))

panel_diag <- tibble(
  df_sfa_origen = df_path,
  n_total_df = nrow(df),
  n_na_NCODI = sum(is.na(df$NCODI)),
  n_na_anyo  = sum(is.na(df$anyo)),
  var_y_total = var_y_total %||% NA_character_,
  var_y_idiag = var_y_idiag %||% NA_character_,
  var_y_q     = var_y_q %||% NA_character_,
  var_y_m     = var_y_m %||% NA_character_,
  var_lnL_c   = var_lnL_c,
  var_lnKC_c  = var_lnKC_c,
  var_lnKT_c  = var_lnKT_c,
  var_ddesc   = var_ddesc,
  var_pctsns  = var_pctsns,
  var_shareq  = var_shareq %||% NA_character_,
  model_name  = MODEL_NAME
)
readr::write_csv(panel_diag, file.path(DIAG_DIR, "00_panel_precheck.csv"))
